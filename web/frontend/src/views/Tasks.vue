  ...
  <div>
    ...
    <div class="create card">
      <div class="type-tabs">
        <button
          class="tab"
          :class="{ active: taskType === 'addanime' }"
          @click="taskType = 'addanime'"
        >
          addanime（已有番剧）
        </button>
        <button
          class="tab"
          :class="{ active: taskType === 'addnewanime' }"
          @click="taskType = 'addnewanime'"
        >
          addnewanime（新番剧）
        </button>
      </div>
      <div class="fields">
        ...
        <input
          v-model="taskUrl"
          placeholder="BT 链接（bangumi.moe / dmhy）"
          style="flex: 1.6"
        />
...
.badge.queued { background: var(--bg-amber); color: var(--amber); }
.badge.running { background: var(--bg-blue); color: var(--blue); }
.badge.done { background: var(--bg-green); color: var(--green); }
.badge.failed { background: #fdebed; color: var(--red); }
.badge.canceled { background: #f1f2f6; color: var(--text-mute); }
.type-badge {
  margin-left: 8px;
  font-size: 11px;
  padding: 3px 9px;
