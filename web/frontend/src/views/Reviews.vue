...
<template>
    <div>
        <div class="head">
            <h2>📝 待确认番剧</h2>
            <div class="tabs">
                <button class="tab flow" :class="{ active: flow === 'pre_post' }" @click="switchFlow('pre_post')">
                    先发后审
                </button>
                <button class="tab flow" :class="{ active: flow === 'pre_review' }" @click="switchFlow('pre_review')">
                    先审核后发
                </button>
            </div>
        </div>
        ...
        <div class="status-tabs">
            <button v-for="s in ['pending', 'approved', 'rejected']" :key="s" class="tab"
                :class="{ active: filter === s }" @click="switchFilter(s)">
