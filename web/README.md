...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `WEB_HOST` | `127.0.0.1` | **监听绑定地址**；仅本机可访问。公网开放需改为 `0.0.0.0` |
| `WEB_PORT` | `3780` | 监听端口 |
| `WEB_PUBLIC_URL` | 空 | 后端**公网对外地址**（域名/公网IP）；`/web` 命令用它展示给前端连接。缺省退回 `http://host:port/` |
| `WEB_API_TOKEN` | 空 | 可选：不过期的主令牌（兼容旧鉴权） |
| `WEB_KEY_SECRET` | 自动 | 密钥签名密钥；不设则持久化到 `web/.web_secret` |

> **公网部署**：监听需 `WEB_HOST=0.0.0.0`（或经反代转发 3780），并用
> `WEB_PUBLIC_URL` 指定公网可访问的地址（如 `https://web.example.com`），
> 否则 `/web` 命令显示的是本机回环地址，前端无法连接。
