        ...
        <div class="flex flex-row w-full items-center justify-between"
            ...
            <div class="flex flex-row items-center" style="display: flex;">
                ...
                <div class="flex items-center justify-center"
                    style="display: flex; width: 40px; height: 40px; border-radius: 50%; overflow: hidden; background-color: #f0f0f0; border: 2px solid #ff5c9d; margin-right: 12px; flex-shrink: 0;">
                    <img v-if="userAvatar" :src="userAvatar"
                        style="width: 40px; height: 40px; object-fit: cover;" alt="Avatar" />
                    <span v-else class="font-bold m-0 p-0" style="font-size: 18px; color: #ff5c9d;">{{ (userName || '?')[0] }}</span>
                </div>
                <!-- User Name + Anime Title -->
                <div class="flex flex-col" style="display: flex; flex-direction: column;">
                    <div class="flex flex-row items-center" style="display: flex;">
                        <span class="font-bold m-0 p-0"
                            style="font-size: 16px; color: #1f1f1f; line-height: 20px;">{{ userName || '未知用户'
                            }}</span>
                        <span v-if="userUsername" class="m-0 p-0"
                            style="font-size: 12px; color: #8c8c8c; margin-left: 6px; line-height: 20px;">/
                            @{{ userUsername }}</span>
                    </div>
                    <div class="flex flex-row items-center" style="display: flex; margin-top: 2px;">
                        <span class="m-0 p-0"
                            style="font-size: 11px; color: #8c8c8c; font-weight: 400; line-height: 14px;">我的动画:</span>
                        <span class="font-bold m-0 p-0"
                            style="font-size: 12px; color: #1f1f1f; margin-left: 4px; line-height: 14px;">{{ name_cn || name
                            }}</span>
                        <span class="font-bold m-0 p-0"
                            style="font-size: 10px; color: #ff5c9d; background-color: rgba(255,92,157,0.08); border: 1.5px solid rgba(255,92,157,0.2); border-radius: 4px; padding: 1px 6px; margin-left: 8px;">{{
                                platform }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-row items-center"
                    style="display: flex; border-radius: 6px; padding: 4px 12px; border: 1.5px solid; background-color: #e6f4ff;"
                :style="watchStatusStyle">
                <span class="font-semibold m-0 p-0" style="font-size: 11px;">{{ watchStatusLabel }}</span>
            </div>
        </div>
        ...
        <div class="flex flex-row w-full" style="display: flex; height: 420px; justify-content: space-between;">
            ...
            <div class="flex flex-col"
                ...
                <div class="flex flex-col" style="display: flex; flex-direction: column; margin-bottom: 6px;">
                    <div class="flex flex-row items-center justify-between" style="display: flex; margin-bottom: 6px;">
                        <div class="flex flex-row items-center" style="display: flex;">
                            <span class="font-bold m-0 p-0"
                                style="font-size: 13px; color: #595959; border-left: 3px solid #1677ff; padding-left: 6px;">我的观看进度</span>
                            <!-- Pagination Tabs -->
                            <div class="flex flex-row" style="display: flex; margin-left: 8px;">
                                <div v-for="p in Math.ceil(episodes.length / episodesPerPage)" :key="p"
                                    ...
                                    :style="{
                                        backgroundColor: (p - 1) === activePageIdx ? '#1677ff' : '#f0f0f0',
                                        border: (p - 1) === activePageIdx ? '1px solid #1677ff' : '1px solid #d9d9d9'
                                    }">
                        ...
                        <span class="font-medium m-0 p-0" style="font-size: 10px; color: #8c8c8c;">进度: {{
                            userWatchingEp }}/{{ episodes.length }}</span>
                    </div>
                    ...
                    <div class="flex flex-row flex-wrap"
                        ...
                        <div v-for="ep in episodes.slice(activePageIdx * episodesPerPage, activePageIdx * episodesPerPage + episodesPerPage)"
                            ...
                            :style="episodeStyles[ep.sort]?.container || 'background-color: #f5f5f5; border: 1.5px solid #e8e8e8;'">
                            <span class="m-0 p-0" style="font-size: 13px; font-weight: 600;"
                                :style="episodeStyles[ep.sort]?.text || 'color: #bfbfbf; font-weight: 400;'">
                    ...
                    <!-- Legend -->
                    <div class="flex flex-row items-center" style="display: flex; margin-bottom: 4px; gap: 12px;">
                        <div class="flex flex-row items-center" style="display: flex; gap: 4px;">
                            <div style="display: flex; width: 10px; height: 10px; border-radius: 3px; background-color: #1677ff;"></div>
                            <span class="m-0 p-0" style="font-size: 9px; color: #8c8c8c;">已看</span>
                        </div>
                        <div class="flex flex-row items-center" style="display: flex; gap: 4px;">
                            <div style="display: flex; width: 10px; height: 10px; border-radius: 3px; background-color: #f5f5f5; border: 1px solid #e8e8e8;"></div>
                            <span class="m-0 p-0" style="font-size: 9px; color: #8c8c8c;">未放映</span>
                        </div>
                        <div class="flex flex-row items-center" style="display: flex; gap: 4px;">
                            <div style="display: flex; width: 10px; height: 10px; border-radius: 3px; background-color: rgba(255,92,157,0.15); border: 1px solid rgba(255,92,157,0.3);"></div>
                            <span class="m-0 p-0" style="font-size: 9px; color: #8c8c8c;">想看</span>
                        </div>
                        <div class="flex flex-row items-center" style="display: flex; gap: 4px;">
                            <div style="display: flex; width: 10px; height: 10px; border-radius: 3px; background-color: #f5f5f5; border: 1px solid #d9d9d9;"></div>
                            <span class="m-0 p-0" style="font-size: 9px; color: #8c8c8c; text-decoration: line-through;">抛弃</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col"
                    style="display: flex; flex-direction: column; background-color: rgba(22,119,255,0.03); border: 1px solid rgba(22,119,255,0.12); border-radius: 8px; padding: 8px 14px; margin-bottom: 10px; box-sizing: border-box; width: 430px;">
                    <span class="font-bold m-0 p-0"
                        style="font-size: 12px; color: #595959; border-left: 3px solid #1677ff; padding-left: 6px; margin-bottom: 6px;">我的完成度</span>
                    <div class="flex flex-row items-center justify-between" style="display: flex;">
                        <div class="flex flex-col"
                            style="display: flex; flex-direction: column; width: 195px; border-right: 1px dashed rgba(22,119,255,0.12); padding-right: 10px; box-sizing: border-box;">
                            <span class="m-0 p-0"
                                style="font-size: 10px; color: #8c8c8c; margin-bottom: 2px; font-weight: 500;">当前观看 第 <span
                                    style="color: #1677ff; font-weight: bold;">{{ userWatchingEp || '-' }}</span> 话</span>
                            <span class="m-0 p-0"
                                style="font-size: 11px; color: #262626; font-weight: 600; font-family: monospace;">{{
                                    completionPercent }}% 完成</span>
                        </div>
                        <div class="flex flex-col"
                            style="display: flex; flex-direction: column; width: 195px; padding-left: 10px; box-sizing: border-box;">
                            <template v-if="userWatchingEp >= episodes.length && episodes.length > 0">
                                <span class="m-0 p-0"
                                    style="font-size: 12px; color: #52c41a; font-weight: 700; letter-spacing: 1px;">
                                    已追完</span>
                            ...
                            <template v-else>
                                <span class="m-0 p-0"
                                    style="font-size: 10px; color: #8c8c8c; margin-bottom: 2px; font-weight: 500;">剩余
                                    <span style="color: #1677ff; font-weight: bold;">{{ episodes.length - userWatchingEp }}</span> 话待观看</span>
                                <span class="m-0 p-0"
                                    style="font-size: 11px; color: #1677ff; font-weight: 600; font-family: monospace;">{{
                                        airedCount - userWatchingEp > 0 ? '可继续追 ' + (airedCount - userWatchingEp) + ' 话' : '等待更新中' }}</span>
                            </template>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col" style="display: flex; flex-direction: column;">
                    <span class="font-bold m-0 p-0"
                        style="font-size: 13px; color: #595959; border-left: 3px solid #722ed1; padding-left: 6px; margin-bottom: 6px;">我的标签</span>
                    <div class="flex flex-row flex-wrap"
                        style="display: flex; flex-wrap: wrap; max-height: 80px; overflow: hidden;">
                        <template v-if="userTags && userTags.length > 0">
                            <div v-for="(tag, idx) in userTags" :key="idx" class="flex flex-row items-center"
                                ...
                                :style="{
                                    backgroundColor: idx === 0 ? 'rgba(114,46,209,0.06)' : '#fbfbfb',
                                    border: idx === 0 ? '1.5px solid rgba(114,46,209,0.2)' : '1.5px solid #d9d9d9'
                                }">
                                <span class="font-medium m-0 p-0" style="font-size: 11px; line-height: 12px;"
                                    :style="{ color: idx === 0 ? '#722ed1' : '#595959' }">{{ tag }}</span>
                            </div>
                        </template>
                        <span v-else class="m-0 p-0" style="font-size: 11px; color: #bfbfbf; font-style: italic;">未添加</span>
                    </div>
                </div>

                <!-- User Rating (10 stars) -->
                <div class="flex flex-col" style="display: flex; flex-direction: column; margin-top: 6px;">
                    <span class="font-bold m-0 p-0"
                        style="font-size: 13px; color: #595959; border-left: 3px solid #faad14; padding-left: 6px; margin-bottom: 6px;">我的评分</span>
                    <div class="flex flex-row items-center" style="display: flex; gap: 3px;">
                        <div v-for="star in 10" :key="star" class="flex flex-row items-center justify-center"
                            style="display: flex; width: 20px; height: 20px;">
                            <span class="m-0 p-0" style="font-size: 16px; line-height: 16px;"
                                :style="{ color: star <= userRating ? '#faad14' : '#d9d9d9' }">★</span>
                        </div>
                        <span class="font-bold m-0 p-0"
                            style="font-size: 14px; color: #faad14; margin-left: 6px;">{{ userRating > 0 ? userRating + '/10' : '-' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col"
                ...
                <div class="flex flex-col w-full"
                    ...
                    <div class="flex flex-row items-center"
                        ...
                        <div class="flex flex-col justify-center"
                            ...
                            <span class="m-0 p-0"
                                style="font-size: 9px; color: #8c8c8c; line-height: 12px; margin-top: 2px;">番剧总评分</span>
                        </div>
                    </div>
                ...
                <div class="flex flex-col w-full"
                    ...
                    <span class="font-bold m-0 p-0"
                        style="font-size: 12px; color: #595959; border-left: 3px solid #ff5c9d; padding-left: 6px; margin-bottom: 8px;">全体用户收藏状态</span>
...
defineProps<{
    ...
    /** 用户 @username */
    userUsername: string
    /** 观影状态标签文本: 在看/看过/想看/搁置/抛弃 */
    watchStatusLabel: string
    /** 观影状态样式字符串 (border + background-color + color) */
    ...
    /** 用户添加的标签列表 */
    userTags: string[]
    /** 每集预计算样式: key=集数sort, value={ container, text } */
    episodeStyles: Record<number, { container: string; text: string }>
}>()
</script>
